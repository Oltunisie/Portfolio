    ...
    <section className="min-h-screen flex flex-col items-center justify-center text-center px-6 relative overflow-hidden bg-gradient-to-br from-slate-100 via-white to-blue-50 dark:from-slate-950 dark:via-[#080d1a] dark:to-slate-900">
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          ...
          className="text-blue-800 dark:text-blue-400 text-sm font-medium tracking-[0.2em] uppercase mb-4">
          Aerospace Engineering · UCLA
        </motion.p>

        <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.3, duration: 0.7 }}
          className="text-6xl md:text-8xl font-light text-slate-900 dark:text-white tracking-tight mb-6">
          Omar Lemkecher
        </motion.h1>

        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          transition={{ delay: 0.5, duration: 0.7 }}
          className="text-xl text-slate-500 dark:text-slate-400 font-light max-w-md mx-auto mb-10">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <button onClick={() => document.getElementById("contact")?.scrollIntoView({ behavior: "smooth" })}
            className="px-8 py-3 border border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-300 text-sm tracking-wide rounded-full hover:border-slate-500 dark:hover:border-slate-400 transition-colors duration-300">
            Get in Touch
          </button>
        </motion.div>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
        ...
        <motion.div animate={{ y: [0, 8, 0] }} transition={{ repeat: Infinity, duration: 1.5, ease: "easeInOut" }}
          className="w-px h-8 bg-gradient-to-b from-slate-400 dark:from-slate-500 to-transparent" />
      </motion.div>
    </section>
  );
}
