...
const achievements = [
  {
    ...
    period: "2023",
    description: "Selected by the French CNES to conduct Zero-G experiments aboard a parabolic flight in Bordeaux, covering thermodynamics, centrifugal forces, and Newton's Laws in microgravity.",
  },
  {
    title: "Young Searchers Prize, Project X",
    period: "2023 · 2024 · 2025",
    description: "Three-time winner — 2023 (Space Probe), 2024 (Zero-G experiments). Selected to present awards at the 2025 closing ceremony.",
  },
  {
...
export default function Experience() {
  ...
  return (
    <section id="experience" className="py-32 px-6 bg-white dark:bg-[#080d1a]">
      <div className="max-w-4xl mx-auto" ref={ref}>
        ...
        <div className="relative">
          {/* Vertical line */}
          <div className="absolute left-[7px] top-0 bottom-0 w-px bg-gradient-to-b from-blue-900/40 via-blue-900/20 to-transparent dark:from-blue-500/40 dark:via-blue-500/20" />
          ...
          <div className="space-y-0">
            {experiences.map((exp, i) => (
              <motion.div key={exp.role}
                ...
                {/* Timeline node */}
                <div className="absolute left-0 top-1.5 flex items-center justify-center w-[15px] h-[15px] rounded-full bg-blue-900 dark:bg-blue-500 ring-4 ring-white dark:ring-[#080d1a]" />

                <div className="flex flex-wrap items-start justify-between gap-2 mb-4">
                  <div>
                    <h3 className="text-slate-900 dark:text-white font-semibold text-lg leading-tight">{exp.role}</h3>
                    <p className="text-blue-800 dark:text-blue-400 text-sm font-medium mt-0.5">{exp.org}</p>
                  </div>
                  <span className="text-xs text-slate-400 dark:text-slate-500 tracking-wide bg-slate-50 dark:bg-slate-800 px-3 py-1 rounded-full mt-1">{exp.period}</span>
                </div>
        ...
        {/* Achievements */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: 0.45 }} className="mt-20">
          ...
          <div className="grid md:grid-cols-3 gap-5">
            {achievements.map((a, i) => (
              <motion.div key={a.title}
                ...
                className="p-6 border border-slate-100 dark:border-slate-800 rounded-2xl hover:border-blue-900/20 dark:hover:border-blue-500/30 hover:shadow-sm dark:hover:shadow-blue-950/30 transition-all duration-300 bg-white dark:bg-slate-900">
                <p className="text-xs text-blue-800 dark:text-blue-400 tracking-widest uppercase mb-2">{a.period}</p>
                <h4 className="text-slate-800 dark:text-slate-100 font-medium text-sm mb-3 leading-snug">{a.title}</h4>
